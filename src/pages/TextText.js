...
export default function TextText() {
  ...
  const handleEncode = async () => {
    try {
      ...
      const res = await axios.post('http://localhost:5000/api/encode/text', formData, { responseType: 'blob' });
  ...
  const handleDecode = async () => {
    try {
      ...
      const res = await axios.post('http://localhost:5000/api/decode/text', formData);
