...
export default function TextAudio() {
  ...
  const handleEncode = async () => {
    try {
      ...
      const res = await axios.post('http://localhost:5000/api/encode/audio', formData, { responseType: 'blob' });
  ...
  const handleDecode = async () => {
    try {
      ...
      const res = await axios.post('http://localhost:5000/api/decode/audio', formData);
